...
function generateDayWiseTime(baseval: number, count: number, yrange: YRange): [number, number][] {
...
function getNewSeries(baseval: number, yrange: YRange) {
...
// ---- Chart Config ----
export const RealtimeLineChart: () => ApexOptions = () => {
  // initialize data only once
  if (data.length === 0) {
    data = generateDayWiseTime(new Date('11 May 2024 GMT').getTime(), 10, {
      min: 10,
      max: 90,
    })
    ...
    chart: {
      ...
      events: {
        mounted: (chart) => {
          ...
          window.setInterval(() => {
            resetData()
            chart.updateSeries([{ data }], false, true)
          }, 60000)
        },
      },
    },
