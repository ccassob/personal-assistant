    ...
    formatter: (params) => {
      ...
      list.forEach((item) => {
        ...
        content += `<div style="margin-top: 4px; padding: 3px 15px;">
                   <span style="display:inline-block;margin-inline-end:5px;border-radius:50%;width:10px;height:10px;background-color:${item.color};"></span>
...
for (let i = 0; i < series.length; ++i) {
  ...
  for (let j = 0; j < data.length; ++j) {
    ...
    const value = typeof data[j] === 'object' && data[j] !== null ? (data[j] as any).value : data[j]
...
export const BarRaceChart = (): EChartsOption => ({
  xAxis: {
    max: 'dataMax',
    axisLine: { lineStyle: { type: 'dashed', color: getColor('light') } },
    axisLabel: { show: true, color: getColor('body-color') },
