...
@Component({
  selector: 'app-morphing-map-bar',
  imports: [Echart],
  templateUrl: './morphing-map-bar.html',
  styles: ``,
})
export class MorphingMapBar implements OnInit, OnDestroy {
    ...
    return this.current === 'map'
      ? {
          tooltip: {
            ...
            formatter: ((p: { name: string; value?: number }) => (p.value != null ? `<strong>${p.name}</strong><br/>Population: ${p.value.toLocaleString()}` : `<strong>${p.name}</strong><br/>No data`)) as echarts.TooltipComponentOption['formatter'],
          },
          visualMap: {
            left: 'right',
            min: 500000,
