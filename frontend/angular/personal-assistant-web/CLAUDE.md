...
ng build                                        # production build to dist/personal-assistant-web/
...
- **State manager** — holds an Angular `signal<LayoutState>` persisted to `sessionStorage` under key `__PERSONAL_ASSISTANT_THEME__`
...
All in `src/app/core/services/api/`, one file per entity plus `DashboardService`. Each exports its TypeScript interface(s) and a `@Injectable({ providedIn: 'root' })` service backed by `HttpClient`. Base URL comes from `API_BASE` in `src/app/constants/index.ts`, which reads `environment.apiBase` — the active environment file is selected by the `ng build`/`ng serve` configuration (`development`, `production`, `docker`, `azure`; see root `CLAUDE.md`). `getApiBase()` layers a `localStorage['__PERSONAL_ASSISTANT_API_URL__']` runtime override on top for the PWA/mobile dynamic-URL flow. See the root `CLAUDE.md` endpoint table for the full controller/route list — it changes often enough that duplicating it here would drift.
